...
public class Util {
    ...
    public static int getRun(int arr[], int probabilty[], int size) {
        int prefix[] = new int[size], i;
        prefix[0] = probabilty[0];
        for (i = 1; i < size; ++i)
            prefix[i] = prefix[i - 1] + probabilty[i];
    ...
    public static String batOrBallDecision(String Team) {
        ...
        if (rand_no == 0)
            return Constant.BATTING;
        else
            return Constant.BALLING;
    }
